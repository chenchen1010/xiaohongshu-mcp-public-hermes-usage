...
这份文档给外部 Hermes / Codex 用户使用，用于接入小红书笔记和评论采集 MCP。

接入后，agent 可以通过公网 Streamable HTTP MCP 调用小红书只读采集能力，包括关键词搜索笔记、获取笔记详情、采集一级评论、采集二级回复、抓取首页推荐流，以及读取用户主页信息。

如果你要采集小红书商品、店铺或关键词电商商品 records，请使用[小红书电商采集 MCP 接入说明](./小红书电商采集-MCP接入说明.md)。
